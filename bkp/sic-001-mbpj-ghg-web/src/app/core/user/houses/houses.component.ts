...
import {
  House,
  MergedHouse,
} from "src/app/shared/services/houses/houses.model";
...
export class HousesComponent implements OnInit, OnDestroy {
  ...
  deleteHouse(row) {
    swal
      ...
      .then((result) => {
        ...
        if (result.value) {
          this.houseService.delete(row.id).subscribe(
            () => {
  ...
  successMessage(type: string) {
    if (type == "register") {
      let title = "Success";
      let message = "House is registered";
      this.notifyService.openToastr(title, message);
    } else if (type == "update") {
      let title = "Success";
      let message = "House is updated";
      this.notifyService.openToastr(title, message);
    } else if (type == "delete") {
      let title = "Success";
      let message = "House is deleted";
      this.notifyService.openToastr(title, message);
    }
  }
}
