...
export class EvaluationSchedulesService {
  ...
  public evaluationSchedulesUrl: string = environment.baseUrl + 'v1/evaluation-schedules/'
  ...
  doRetrieveAllEvaluationSchedules(): Observable<EvaluationSchedule[]> {
    ...
    return this.http.get<EvaluationSchedule[]>(this.evaluationSchedulesUrl, {headers: headers}).pipe(
      tap((res) => {
  ...
  doCreateEvaluationSchedule(credentials: Form): Observable<any> {
    let headers = this.createHeader()
    return this.http.post<any>(this.evaluationSchedulesUrl, credentials, {headers: headers}).pipe(
      tap((res) => {
  ...
  doRetrieveFilteredEvaluationSchedules(filterField): Observable<any> {
    ...
    return this.http.get<EvaluationSchedule[]>(filterUrl, {headers: headers}).pipe(
      tap((res) => {
