...
export class EvaluationsService {
  ...
  private urlEvaluationExtended: string = environment.baseUrl + 'v1/evaluations/extended/'

  // Data
  public evaluation: Evaluation
  public evaluationExtended: Evaluation
  public evaluations: Evaluation[] = []
  public evaluationsExtended: Evaluation[] = []
  ...
  getAllExtended() {
    return this.http.get<Evaluation[]>(this.urlEvaluationExtended).pipe(
      tap((res: Evaluation[]) => {
        this.evaluationsExtended = res
        console.log('Evaluation: ', this. evaluationsExtended)
      })
    )
  }
  ...
  getSingleExtended(id: number) {
    let urlTemp = this.urlEvaluationExtended + id + '/'
    return this.http.get<Evaluation>(urlTemp).pipe(
      tap((res: Evaluation) => {
  ...
  filter(field: string) {
    let urlTemp = this.urlEvaluationExtended + '?' + field + '/'
    return this.http.get<Evaluation[]>(urlTemp).pipe(
      tap((res: Evaluation[]) => {
